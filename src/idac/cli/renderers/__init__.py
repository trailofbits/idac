...
def _render_list_rows(
    ...
    if not isinstance(value, list) or not value:
        return "none"
    return "\n".join(render_row(item) for item in value)


def _append_section(lines: list[str], label: str, rows: list[str]) -> None:
    if not rows:
        return
...
def _function_header(value: dict[str, Any]) -> str:
    return f"{_get_first_present(value, 'function')} @ {_get_first_present(value, 'address')}"
...
def render_target_list(value: Any) -> str:
    def render_row(item: Any) -> str:
        ...
        record_id = _get_first_present(item, "record_id")
        state = _get_first_present(item, "state")
        details: list[str] = []
        backend = item.get("backend")
        if backend not in (None, ""):
            details.append(str(backend))
        path = item.get("idb_path") or item.get("exe_path")
        if path not in (None, ""):
        ...
        suffix = f" [{state}]"
        return f"{record_id}{suffix}" + (f" ({', '.join(details)})" if details else "")
...
def render_doctor(value: Any, *, color: bool = False) -> str:
    ...
    lines: list[str] = [
        ...
        f"healthy: {_ansi(str(healthy), _BOOL_COLORS[healthy], color=color)}",
    ]
...
def render_comment(value: Any) -> str:
    ...
    return "none" if comment in (None, "") else str(comment)
...
def render_bookmarks(value: Any) -> str:
    ...
    return "\n".join(_list_rows(rows, render_row))
...
def render_class_hierarchy(value: Any) -> str:
    ...
    for label, items in (
        ...
        rendered = ", ".join(str(item) for item in items) if items else "none"
...
def render_type_show(value: Any) -> str:
    ...
    kind = value.get("kind")
    if name or kind:
        header = "<unknown>" if name in (None, "") else str(name)
        if kind:
            header += f"  {kind}"
        lines.append(header)
...
def _render_type_diagnostic(item: dict[str, Any]) -> str:
    line = item.get("line")
...
def render_python_exec(value: Any) -> str:
    if not isinstance(value, dict):
        return _fallback(value)
    output = "".join(str(value.get(key) or "") for key in ("stdout", "stderr"))
    rendered_result = value.get("result_repr")
    if output and rendered_result:
...
TEXT_RENDERERS: dict[str, Callable[[Any], str]] = {
    ...
    "type_deps": render_type_show,
    "type_declare": render_type_declare,
    "type_declare_check": render_type_declare,
