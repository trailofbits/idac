...
- `function list` text output includes a type column: `real`, `thunk`, or `import`
...
- JSON `function list` rows include `type`
...
For broad function discovery, prefer `function list "name1|name2" --regex -i` so IDA filters before rendering. Avoid producing a full function list just to pipe it into `rg`; add `--out <path>` if the filtered result is still too large for inline output.
