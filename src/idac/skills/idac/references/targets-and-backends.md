...
If the bridge plugin is missing or stale, run `idac setup update --component plugin`. Ask the user to reload the plugin in IDA — idac cannot reload it — before retrying discovery. `setup update` refreshes existing targets and installs any missing targets.
