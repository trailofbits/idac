...
def test_nexus_local_list(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
) -> None:
    payload = run_nexus_json(idac_cmd, idac_env, copy_database(tiny_database), "function", "locals", "list", "main")
...
def test_nexus_local_id_remains_valid_after_rename(
    ...
    listed = run_nexus_json(idac_cmd, idac_env, database, "function", "locals", "list", "main")
    target = next(item for item in listed["locals"] if isinstance(item, dict) and item.get("name") == "v4")

    renamed = run_nexus_json(
        idac_cmd,
        idac_env,
        database,
        "function",
    ...
    assert isinstance(renamed, dict)
    assert any(item["name"] == "sum_value_again" for item in renamed["locals"] if isinstance(item, dict))


def test_nexus_local_update_accepts_stable_selectors_without_positional_selector(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
...
def test_nexus_local_rename_accepts_stable_selector_without_positional_selector(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
...
def test_nexus_local_retype_accepts_stable_selector_without_positional_selector(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
...
def test_nexus_local_retype_accepts_type_shorthand(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
