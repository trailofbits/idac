...
from tests.helpers import preview_round_trip, run_nexus_json
...
def test_bookmark_list_returns_all_live_slots_only(
    ...
    run_nexus_json(idac_cmd, idac_env, database, "bookmark", "set", "5", "main", "--comment", "entry point")
    run_nexus_json(idac_cmd, idac_env, database, "bookmark", "set", "2", "add", "--comment", "helper")

    result = run_nexus_json(idac_cmd, idac_env, database, "bookmark", "list")
...
def test_bookmark_add_uses_lowest_free_slot(
    ...
    added = run_nexus_json(idac_cmd, idac_env, database, "bookmark", "add", "main", "--comment", "auto slot")
    ...
    assert run_nexus_json(idac_cmd, idac_env, database, "bookmark", "show", "0") == {
        "slot": 0,
        "present": True,
        "address": "0x100000460",
        "comment": "auto slot",
...
def test_bookmark_set_preview_then_persist_overwrites_existing_slot(
    ...
    database = copy_database(tiny_database)
    run_nexus_json(idac_cmd, idac_env, database, "bookmark", "set", "5", "add", "--comment", "helper")
...
def test_bookmark_add_preview_then_persist_round_trip(
    ...
    assert after_preview == {"bookmarks": [], "count": 0}
    assert persisted == {
        "slot": 0,
        "present": True,
        "address": "0x100000460",
...
def test_bookmark_delete_empty_slot_is_a_noop(
    ...
    deleted = run_nexus_json(idac_cmd, idac_env, database, "bookmark", "delete", "5")
