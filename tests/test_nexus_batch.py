...
def test_batch_reuses_batch_dir_and_updates_prototypes_and_locals(
    ...
    (batch_dir / "recovery.idac").write_text(
        "\n".join(
            [
                "# batch commands inherit the wrapper's Nexus target",
    ...
    updated_proto = run_nexus_json(idac_cmd, idac_env, database, "function", "prototype", "show", "add")
    updated_locals = run_nexus_json(idac_cmd, idac_env, database, "function", "locals", "list", "main")
    type_info = run_nexus_json(idac_cmd, idac_env, database, "type", "show", "cli_batch_record")
...
def test_batch_defaults_to_stdout_json_without_out(
    ...
    assert payload["results"][0]["result"]["address"]
    assert "add(" in payload["results"][0]["result"]["prototype"]
...
def test_batch_lint_fixture_context_reports_command_local_errors(
    ...
    diagnostic = payload["results"][0]["stderr"]
    assert "rename" in diagnostic
...
def test_batch_reports_nexus_operation_failure(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
    ...
    assert "missing_symbol" in proc.stderr
    payload = json.loads(out_path.read_text(encoding="utf-8"))
    assert payload["ok"] is False
    assert payload["commands_failed"] == 1
    assert "missing_symbol" in payload["results"][0]["stderr"]
