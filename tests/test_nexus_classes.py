...
from tests.helpers import run_nexus_json
...
def test_nexus_class_list_and_show_from_fixture(
    idac_cmd: list[str],
    ...
    list_payload = run_nexus_json(idac_cmd, idac_env, database, ["type", "class", "list", "Handler"])
    ...
    show_payload = run_nexus_json(idac_cmd, idac_env, database, ["type", "class", "show", "Handler_Stream"])
...
def test_nexus_class_hierarchy_fields_and_vtable_from_fixture(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    handler_hierarchy_stripped_database: Path,
    ...
    hierarchy_payload = run_nexus_json(idac_cmd, idac_env, database, ["type", "class", "hierarchy", "Handler"])
    ...
    vtable_payload = run_nexus_json(idac_cmd, idac_env, database, ["type", "class", "vtable", "Handler_Stream"])
...
def test_nexus_type_declare_replace_with_fixture_class_header(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    handler_hierarchy_stripped_database: Path,
...
def test_nexus_class_candidates_finds_symbols_in_unstripped_fixture(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    ...
    payload = run_nexus_json(idac_cmd, idac_env, database, ["type", "class", "candidates", "Handler_Stream"])
...
def test_nexus_class_candidates_kind_filter_limits_results(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    handler_hierarchy_database: Path,
...
def test_nexus_class_vtable_runtime_includes_raw_targets(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    handler_hierarchy_database: Path,
