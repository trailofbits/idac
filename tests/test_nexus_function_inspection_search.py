...
def test_nexus_function_frame(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
) -> None:
    payload = run_nexus_json(idac_cmd, idac_env, copy_database(tiny_database), "function", "frame", "main")
...
def test_nexus_function_stackvars(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
) -> None:
    payload = run_nexus_json(idac_cmd, idac_env, copy_database(tiny_database), "function", "stackvars", "main")
...
def test_nexus_function_callees_and_xrefs(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
    ...
    callees = run_nexus_json(idac_cmd, idac_env, database, "function", "callees", "main")
    ...
    callers = run_nexus_json(idac_cmd, idac_env, database, "function", "callers", "add")
    ...
    xrefs = run_nexus_json(idac_cmd, idac_env, database, "xrefs", "add")
    assert xrefs == [
        {
            "from": "0x100000478",
            "kind": "call",
...
def test_nexus_search_bytes_supports_ida_hex_pattern(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
...
def test_nexus_search_bytes_supports_ida_wildcards(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
...
def test_nexus_search_strings_respects_segment_scope(
    idac_cmd: list[str],
    idac_env: dict[str, str],
    copy_database,
    tiny_database: Path,
